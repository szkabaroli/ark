...
use bytecode::{
    BytecodeBuilder, BytecodeFunction, BytecodeType, BytecodeTypeArray, ConstPoolIdx, FunctionId,
    Intrinsic, Label, Location, Register,
};
...
pub fn generate_fct(sa: &Sema, fct: &FctDefinition, src: &AnalysisData) -> BytecodeFunction {
    let bytecode_generator = CodeGen {
        sa,
    ...
    bytecode_generator.generate_fct(&fct.ast)
}
...
impl<'a> CodeGen<'a> {
    ...
    fn visit_expr_assert(&mut self, expr: &ast::Call, dest: DataDest) {
        assert!(dest.is_unit());
    ...
    fn visit_expr_call(&mut self, expr: &ast::Call, dest: DataDest) -> Register {
    ...
    fn visit_expr_un(&mut self, expr: &ast::ExprUnOpType, dest: DataDest) -> Register {
    ...
    fn visit_expr_bin(&mut self, expr: &ast::BinOp, dest: DataDest) -> Register {
    ...
    fn visit_expr_assign_var(&mut self, expr: &ast::BinOp, var_id: VarId) {
        let var = self.analysis.vars.get_var(var_id);
    ...
    fn visit_expr_assign_call(&mut self, expr: &ast::BinOp, call_expr: &ast::Call) {
    ...
    fn visit_expr_assign(&mut self, expr: &ast::BinOp, dest: DataDest) -> Register {
        assert!(dest.is_unit());
        ...
        if expr.lhs.is_ident() {
            ...
            match *expr.lhs {
                ...
                ast::ExprKind::Call(ref call) => self.visit_expr_assign_call(expr, call),
                _ => unreachable!(),
            };
        }
    ...
    fn emit_bin_or(&mut self, expr: &ast::BinOp, dest: DataDest) -> Register {
        if dest.is_effect() {
    ...
    fn emit_bin_is(&mut self, expr: &ast::BinOp, dest: DataDest) -> Register {
        if dest.is_effect() {
    ...
    fn emit_expr_for_effect(&mut self, expr: &ast::ExprKind) {
        let reg = gen_expr(self, expr, DataDest::Effect);
        self.free_if_temp(reg);
    }

    fn emit_bin_and(&mut self, expr: &ast::BinOp, dest: DataDest) -> Register {
        if dest.is_effect() {
            let end_lbl = self.builder.create_label();
            let dest = gen_expr(self, &expr.lhs, DataDest::Alloc);

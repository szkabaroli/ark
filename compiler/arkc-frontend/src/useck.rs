...
use crate::hir::{module_package, ModuleDefinitionId};
use crate::Sema;
use parser::ast::*;
use parser::Span;

use super::hir::SourceFileId;
...
pub fn check<'a>(sa: &Sema, mut module_symtables: HashMap<ModuleDefinitionId, SymTable>) {
    ...
    while {
        ...
        for (_id, import_definition) in &sa.imports {
            let _ = check_import(
                sa,
                &mut module_symtables,
                &import_definition.ast,
                import_definition.module_id,
                import_definition.file_id,
                import_definition.visibility,
                None,
                true,
                &mut processed_uses,
    ...
    for (_id, import_def) in &sa.imports {
        let _ = check_import(
            sa,
            &mut module_symtables,
            &import_def.ast,
            import_def.module_id,
            import_def.file_id,
            import_def.visibility,
            None,
            false,
            &mut processed_uses,
            &mut false,
...
fn check_import(
    ...
    import_module_id: ModuleDefinitionId,
    ...
    if processed_uses.contains(&(import_file_id, import_path.id)) {
        return Ok(());
    }
    ...
    for (idx, component) in import_path.path.iter().enumerate().skip(start_idx) {
        if !previous_sym.is_enum() && !previous_sym.is_module() {
            let msg = ErrorMessage::ExpectedPath;
            sa.report(import_file_id, import_path.path[idx - 1].span, msg);
            assert!(processed_uses.insert((import_file_id, import_path.id)));
            return Err(());
        }
    ...
    match &import_path.target {
        UsePathDescriptor::Default => {
            let last_component = import_path.path.last().expect("no component");
            assert!(processed_uses.insert((import_file_id, import_path.id)));

            let name = match last_component.value {
                ImportPathComponentValue::Name(ref name) => name.clone(),
                ImportPathComponentValue::Package
                | ImportPathComponentValue::Super
                | ImportPathComponentValue::This
        ...
        UsePathDescriptor::Group(ref group) => {
            if group.targets.is_empty() {
                sa.report(import_file_id, group.span, ErrorMessage::ExpectedPath);
                assert!(processed_uses.insert((import_file_id, import_path.id)));
                return Err(());
            }
...
fn initial_module(
    ...
    if let Some(first_component) = use_path.path.first() {
        match first_component.value {
            ImportPathComponentValue::This => Ok((1, SymbolKind::Module(use_module_id))),
            ImportPathComponentValue::Package => {
            ...
            ImportPathComponentValue::Name(ref ident) => {
            ...
            ImportPathComponentValue::Error => Err(()),
        }
    } else {
        Err(())
    }

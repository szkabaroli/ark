...
use crate::sema::{FctDefinitionId, Sema, SourceFileId};
use crate::specialize::{replace_type, AliasReplacement};
...
impl<'a> MethodLookup<'a> {
    ...
    pub fn find(&mut self) -> MethodLookupResult {
        ...
        let fct = self.sa.function(fct_id);
        ...
        if args.contains(&SourceType::Unknown) {
            return result;
        }
